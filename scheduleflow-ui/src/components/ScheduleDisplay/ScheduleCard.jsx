...
const getEarliestAndLatestTime = (schedule) => {
  const scheduleTimes = schedule.reduce(
    (acc, node) => {
      const nodeStartTime = new Date(node.startTime);
      const nodeEndTime = new Date(node.endTime);
      return {
        earliestTime:
          nodeStartTime < acc.earliestTime ? nodeStartTime : acc.earliestTime,
        latestTime: nodeEndTime > acc.latestTime ? nodeEndTime : acc.latestTime,
      };
    },
    {
      earliestTime: new Date(Date.UTC(1970, 0, 1, 23, 59, 0)),
      latestTime: new Date(Date.UTC(1970, 0, 1, 0, 0, 0)),
    }
  );

  scheduleTimes.earliestTime = `${String(
    scheduleTimes.earliestTime.getUTCHours()
  ).padStart(2, "0")}:${String(
    scheduleTimes.earliestTime.getUTCMinutes()
  ).padStart(2, "0")}`;
  scheduleTimes.latestTime = `${String(
    scheduleTimes.latestTime.getUTCHours()
  ).padStart(2, "0")}:${String(
    scheduleTimes.latestTime.getUTCMinutes()
  ).padStart(2, "0")}`;
  ...
  return (
    <div
      ...
      <div className="flex flex-col h-full justify-center">
        <div
          className={
            "radial-progress text-xl font-extrabold shadow-md " +
            getRatingColor(currRating)
          }
          style={{
            "--value": `${currRating === "N/A" ? 0 : (currRating / 5) * 100}`,
            "--size": "5.5rem",
            "--thickness": "0.35rem",
          }}>
