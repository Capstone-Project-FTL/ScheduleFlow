...
export default function ScheduleContentGrid() {
  ...
  const times = [{startTime: new Date(1970, 0, 1, 7, 30, 0, 0), endTime: new Date(1970, 0, 1, 7, 40, 0, 0)},
  ...
  return (
    <div className="schedule-content-grid grid grid-cols-6 grid-flow-col-dense divide-x-2 divide-gray-300 text-xl text-black w-full ">
      ...
      {["Mon", "Tue", "Wed", "Thu", "Fri"].map((_, idx) => (
        <div
          className={`time-span relative grid grid-flow-row divide-y-2 divide-gray-300 overflow-clip`}
          id={`${idx}`}
        >
          {Array(numOfIterations)
            .fill(0)
            .map((_, i) => (
              <p className="h-10">
                {times.map((e) => <EventCard start={e.startTime} end={e.endTime} />)}
              </p>
            ))}
        </div>
      ))}
